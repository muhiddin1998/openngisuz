...
def process_cadastre_request(chat_id, cadastre_number, api_url, cat_title):
    ...
    try:
        ...
        if 'features' in data and len(data['features']) > 0:
            ...
            bot.send_message(chat_id, f"[-] '{cadastre_number}' raqami bo'yicha {cat_title} bazasidan ma'lumot topilmadi. Raqam to'g'riligini va tanlangan kategoriya mosligini tekshiring.", reply_markup=get_main_keyboard())
            
    except requests.exceptions.Timeout:
        bot.send_message(chat_id, "[!] Server vaqtincha javob bermayapti, iltimos birozdan keyin qayta urinib ko'ring.", reply_markup=get_main_keyboard())
